...
- **In a session worktree, a `cherry-pick`/`revert`/`rebase` that exits 0 has
  ...
  MEASURED 2026-09-11: `--abort`, `--quit`, `--continue` and `--skip` all
  leak, and so does a plain `git cherry-pick <sha>` that never conflicted;
  `git merge --abort` and `git am --abort` are clean. The L1 git shim now
  refuses such a run (exit 1) and prints the `rm -rf` that ends it — read its
  recipe, run it, then re-read `git status`. Doing it by hand: remove
  `CHERRY_PICK_HEAD`, `REVERT_HEAD`, `AUTO_MERGE` and `MERGE_MSG`, and
  `rm -rf sequencer`, all under the dir `git rev-parse --git-dir` names.
