...
|---|---|---|---|
| `keychain item "Claude Code-credentials" unreadable` | the keychain ACL is per-binary, and every promotion of a new posse binary can drop this one's read | grant access when macOS prompts, or run `claude` once | not a stale token — do **not** "refresh" |
| `usage endpoint returned 401 Unauthorized: credential stale — waiting for an interactive refresh` | the OAuth token is stale; the read reached the endpoint and the endpoint said "not you" | run `claude` once and let it log in; its own next launch refreshes it too | not an outage, and not weather — waiting alone does not clear it |
| `usage endpoint returned 403 Forbidden: this credential is not entitled to plan windows — not a freshness problem` | the wrong **kind** of credential reached the endpoint — a setup-token pointed at `/api/oauth/usage`. Measured, not guessed | stop. Take the setup-token back out of the guard's path | **never** "run `claude` once to refresh": refreshing a credential that was never entitled returns the same 403 forever |
