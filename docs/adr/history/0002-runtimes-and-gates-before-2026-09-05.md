...
Archived 2026-09-05. Current decision: [ADR 0002](../0002-runtimes-and-gates.md).
...
launcher's land path belts the same class, recorded as a note, not a tier) · amended 2026-09-02 (ADR 0042, ranger-base-eje6d: §3 L1 row — the typed-line prepend also puts the runtime process's own execs inside the wall, on purpose: a crew runtime authenticates with posse's session mint and never with the operator's keychain, and the shim in front of its own credential read is what enforces that at `shims`; §4's caged credential precondition generalizes to every launch whose PID shims `Runtime.CredBin`)*
...
| L1 shims | `shims` (always on) | `gates/<p>/bin/<cmd>` for every `Bash(<cmd> <prefix>:*)`/`Bash(<cmd>)` deny; refuses when argv matches, else `exec`s the real binary resolved at render time; logs refusals to `gates/<p>/refusals.log`; PATH is prepended **on the typed command line** (`PATH=<bin>:$PATH <cmd>`), not in the workspace env, because macOS `path_helper` reorders PATH when the pane shell starts — and, since ADR 0042, because the runtime process's own execs belong inside the wall: a crew runtime authenticates with the session mint posse injects and never with the operator's keychain, and the shim in front of its own credential read (`Runtime.CredBin`) is what enforces that at this tier; a launch whose PID shims that binary requires the mint in its env set, else refuses (§4's caged precondition, generalized) | any deny that is a shell verb, on any runtime |
...
| L2 seatbelt | `seatbelt` | `sandbox-exec -f gates/<p>/seatbelt.sb` wrapping the rendered command: deny `file-write*` except cwd, `{memory}`, the **launching** runtime's own state dir and no other runtime's (its `state_dir:`, ADR 0012 D4 — so a claude launch grants `~/.claude`/`~/.claude.json` and NOT `~/.codex`/`~/.grok`; it granted the union of all three until ranger-base-9fl, and write access to another runtime's auth store is an exfil channel that outlives the session that plants it — and where a `state_dir:` names a FILE, its atomic-write siblings `<p>.lock` and `<p>.tmp.*` as a dot-anchored `regex`, because `subpath` is component-aware and a grant on the file alone loses every write while the CLI reports success, ranger-base-cypy1), `$TMPDIR`; PID may add `writable:` paths; parametrized `Edit(<glob>)` is a trailing `subpath` deny after that allow (ADR 0014; last match wins) | `Edit`/`Write`-class denies on any runtime — bare and subtree-glob; the only runtime-proof file gate |
...
then `cage: container` refuses with that reason. ADR 0042 generalizes it: any launch whose PID shims the runtime's own credential binary (`Runtime.CredBin`) requires the same mint, at every tier, and refuses without it.
