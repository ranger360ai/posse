...
- **QA**: *Your checklist is the "done when" in the closing persona's PID row for this intent plus the bead's acceptance text; verify against the closing commit(s), not the description. Misses are ONE findings bead per verify close, `-l debt`, one line per finding with its repro (HANDOFF; ADR 0006 §1 as amended 2026-09-02 — a live money / constitution / dispatch defect alone gets its own `-t bug` bead), and the closed bead is reopened only by the operator.*
