...
| `ranger-base-dvxac` — re-ask `/api/oauth/usage` outside the rate-limit window | 09-02 05:25 → 09-02 13:36 | **yes, venue**: the experiment needs an hour in which nothing else on the box asks, which the deciding session cannot manufacture | **yes** — `wkai3 --blocks--> dvxac` (removed before close, and the close comment says why) |
