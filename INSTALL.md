...
| symptom | cause | fix |
|---|---|---|
| `posse init` prints `(seed: <dir>/examples)` where you expected `(seed: embedded)` | a real seed tree — `config.yaml` with `agents/`, `recipes/` and `envs/` — sits one level above the binary and wins over the embed: right in a dev build, wrong anywhere else | move that directory aside and re-run `posse init`; it overwrites nothing, so the files the wrong seed missed fill in. Read the re-run's second line: `filled <n> missing seed file(s) and re-stamped ... (seeded)` means the manifest followed them and the home launches. If the re-run says nothing about `promoted.json`, this home was **promoted**, not seeded — the filled-in files are outside `promoted.json` and every *dispatched* launch now refuses (ADR 0015 §3); `posse promote` again is what clears it (ranger-base-pith) |
