...
- **In a session worktree, a `cherry-pick`/`revert`/`rebase` that exits 0 has
  ...
  `.git/packed-refs.lock`. The session's sandbox may create and remove that
  lock, so the ordinary run is clean and silent. What still bites is a lock
  git cannot HAVE: one someone else left stranded there — an operator `gc`, a
  git killed mid-transaction — or an L4 cage, which binds none. git then waits
  a second per ref transaction, three of them, gives up the delete and
  **exits 0 anyway**: a few seconds of silence and a zero exit. The tree is
  ...
  this, not your PID.** Measured with the lock unavailable: `--abort`,
  `--quit`, `--continue` and `--skip` all leak, and so does a plain
  ...
  operator's to remove, never a session's: git's lock files carry no holder
  identity, so a session cannot tell a stranded one from a live `gc`'s.
...
- **`BEADS_DIR` is set in your session, and it is why the graph does not
  fork.** Every session posse launches carries
...
- **Ending anything: `kill` one pid, never a `pkill -f` pattern.** `pkill -f`
  ...
  name.
  **Enforced, not advised** (operator ruling 2026-09-03): every crew PID denies
  `Bash(pkill:*)` and `Bash(killall:*)` beside `Bash(git push:*)`, realized as
  PATH shims that refuse the whole verb, so a `pkill` or `killall` typed at a
  crew seat is refused by the gate whatever its pattern — the session-unique
  spellings above included, `-P $$` among them. Keep the pid the launcher
  printed and `kill` that, or `kill -- -$$` for your own process group;
...
whatever the caps say — and from the first blind pass, not after the
`plan_guard_blind_max:` grace, which is tolerance for a pass that knows
nothing rather than for one holding a reading that already said stop.
Dollars are not a brake on your plan's weekly window, and neither is a
clock. Do not read a spend cap as cover for a meter you cannot read.
