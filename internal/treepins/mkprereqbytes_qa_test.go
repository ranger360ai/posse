...
// every treepins `*_test.go` file — is where mkPrereqs, mkRuns and every
...
// mkpFile is one parsed treepins test file.
type mkpFile struct {
	rel  string
	file *ast.File
}

// mkpCorpus parses every treepins *_test.go file — the tree mkPrereqs,
// mkRuns and every known call site of this defect live in.
func mkpCorpus(t *testing.T) (*token.FileSet, []mkpFile) {
	t.Helper()
	paths, err := filepath.Glob("internal/treepins/*_test.go")
	if err != nil {
		t.Fatal(err)
	}
	// Reader sanity: a walk that opened nothing would pass the sweep below
	// on silence. 47 treepins *_test.go files on 2026-09-06; loose on
	// purpose — this floor catches a broken glob, not a file count.
	if len(paths) < 40 {
		t.Fatalf("only %d treepins *_test.go files found — the glob is broken, not the tree", len(paths))
	}
