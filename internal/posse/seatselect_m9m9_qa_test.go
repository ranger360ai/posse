...
//
// These four started life as reviewer's green pins asserting the hole
...
func TestQASeatSelectionMissingSoAFreeSeatIsNeverOffered(t *testing.T) {
	...
	// The two shapes a busy SKIP can take. "developer busy" inside a seat
	...
	// ADR 0020 §4: a persona stays strictly serial. The trailing comma is
	// load-bearing — the seat names share a prefix, and `persona developer`
	// alone also counts `persona developer-2, …`.
...
// lane busy: developer, developer-2, developer-3'), not one persona."
//
// The line the single-seat model printed named Route's head, which reads as
// "developer IS the code lane". Naming the lane is also the hiring signal ADR
...
func TestQALaneBusySkipStillNamesOnePersonaNotTheLane(t *testing.T) {
	...
	if !strings.Contains(out, "code lane busy: developer, developer-2") {
...
// (seat 2/3: developer-2; developer busy)' — the audit line ranger-base-2yj5 asked
// for."
//
...
func TestQARouteWhyNamesTheRaceNotTheSeat(t *testing.T) {
	...
	writePersona(t, b.App, "developer", "[code]")
	writePersona(t, b.App, "developer-2", "[code]")
	writePersona(t, b.App, "developer-3", "[code]")
	qaRepo(t, b.App,
		`[{"id":"a-1","title":"t","labels":["code"]},{"id":"a-2","title":"u","labels":["code"]}]`,
		`[{"id":"x","status":"closed"}]`)
	agentPerLaunch(t, fake)
	...
	if !strings.Contains(out, "developer busy") {
	...
	if p != "developer" || !strings.Contains(why, "first of 3: developer, developer-2, developer-3") {
...
func TestQAPersonaFilterSilentlyDropsABeadInThatPersonasLane(t *testing.T) {
	...
	n, err := d.Run("", "developer-2", 0)
	if err != nil {
		t.Fatal(err)
	}
	out := dispatcherOut(d)
	if n != 1 {
		t.Errorf("developer-2 is in a-1's lane, so --persona developer-2 must seat it there, got %d:\n%s", n, out)
	}
	if !strings.Contains(out, "developer-2") {
...
func TestQAPersonaFilterSkipsBeadsOutsideTheLane(t *testing.T) {
	...
	d := newTestDispatcher(t, b)
	writePersona(t, b.App, "developer", "[code]")
	writePersona(t, b.App, "devops", "[infra]")
	qaRepo(t, b.App, `[{"id":"a-1","title":"t","labels":["code"]}]`, `[{"id":"x","status":"closed"}]`)
	agentPerLaunch(t, fake)

	n, err := d.Run("", "devops", 0)
	if err != nil {
		t.Fatal(err)
	}
	out := dispatcherOut(d)
	if n != 0 {
		t.Errorf("devops is not in a-1's lane; --persona devops must dispatch nothing, got n=%d:\n%s", n, out)
	}
	if strings.Contains(out, "a-1") {
		t.Errorf("a bead outside the filtered persona's lane gets no line of its own:\n%s", out)
	}
	if !strings.Contains(out, "outside devops's lane") {
