...
func LandingPrompt(m *HerdrMeta) string {
	...
	if m.Agent != "" {
		b.WriteString("- Append this session's durable lessons to your standing orders ($POSSE_PERSONA_DIR/ORDERS.md): codebase gotchas, commands that work, conventions learned. Skip what is already written there.\n")
	}
...
func KillLandingPrompt(m *HerdrMeta) string {
	...
	if m.Agent != "" {
		b.WriteString("- Append this session's durable lessons to your standing orders ($POSSE_PERSONA_DIR/ORDERS.md): codebase gotchas, commands that work, conventions learned. Skip what is already written there. Do NOT commit that file — posse commits it for you once this turn settles.\n")
	}
	b.WriteString("- Commit work in progress with a clear message, and record on every bead you touched what changed and why (`bd comments add <id> <note>`).\n")
