...
var runtimeFieldAudit = map[string]runtimeFieldNote{
	...
	"FleetSettings":      {fcConsumed, []string{"runtime.go", "agents.go"}, []string{"FleetSettingsText"}, "what {settings} renders: the one flag whose VALUE posse computes at launch, because it carries the credential-dir pin and that is a property of the box (ranger-base-rq83c)"},
	"SettingsPin":        {fcConsumed, []string{"runtime.go", "agents.go"}, []string{"EnsureSettingsPin"}, "EnsureSettingsPin appends the credential-dir pin ALONE to a line that carries no settings flag of its own — the security guarantee reaches a hand-written command:, posse's unattended policy does not"},
